...
app.directive('myCurrentTime', ['$interval', 'dateFilter', function($interval, dateFilter) {
		...
		return function(scope, element, attrs) {
			...
			function updateTime() {
				...
				if (flagIEClock === true) {
					setClock(); //IE analog to digital clock
				}
			}
...
app.factory('myAppFactoryJson', function(){
	//Note: String characters that must be escaped within a JSON string, according to RCF 7159, are listed here: https://tools.ietf.org/html/rfc7159#section-7
	/*
...
<script type="text/javascript"> /* Site-wide jQuery, Angular, Bootstrap */
...
function getBrowserClock() {
	// this function setermins which style of clock to use. My svg clock is no 100% on IE so, this function will call a different clock display instead.
	var isIE = /*@cc_on!@*/false || !!document.documentMode;
	...
	if (isIE) {
		...
		return "<svg width='300px' height='300px' viewBox='0 0 300 300'><use xlink:href='#myClockSvgId' ng-if = 'true'></use></svg>"; // browser DOES SUPPORT my alaog clock svg
	}
}
